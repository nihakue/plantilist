...
export function Landing() {
  ...
  return (
    <div>
      ...
      <div>
        <a href="javascript:(function(){if (!window.location.host.includes('pinterest')) {return;}const path = window.location.pathname;if (path.split('/').length === 4) {window.location.href = `https://plantilist.club/boards${path}`;}})();">Open in Plantilist</a><span> -- Drag this link into your bookmarks bar and click on it when you're viewing a board in pinterest to load it in Plantilist!</span>
      </div>
    </div>
  )
}
